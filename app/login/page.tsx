...
import { Compass, Mail, Lock, ShieldAlert, Loader2, Eye, EyeOff } from "lucide-react";
  ...
  return (
    <div className="w-full max-w-md glass-panel p-8 border-rose-500/20 bg-slate-900/10 text-left shadow-2xl relative animate-slide-up">
      <div className="text-center mb-6">
        ...
        <p className="text-xs text-slate-400 mt-1">Access your trips and manage shared expense splits.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <div className="flex items-center gap-2 bg-slate-950 border border-rose-500/20 rounded-xl px-3.5 py-2.5 w-full">
        ...
        <div>
          ...
          <span className="block text-[9px] text-slate-500 mt-1.5 leading-relaxed">
            💡 For shell accounts generated via WhatsApp, the default password is <code className="text-rose-400 font-semibold font-mono">password</code>.
          </span>
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-3.5 bg-gradient-to-r from-rose-400 to-teal-400 hover:from-rose-450 hover:to-teal-350 text-slate-950 font-extrabold text-xs rounded-xl shadow-lg transition duration-200 mt-2 cursor-pointer text-center flex items-center justify-center gap-1.5"
        >
          {loading ? (
            <>
              <Loader2 className="animate-spin" size={13} /> Logging in...
            </>
          ) : (
            "Log In"
          )}
